...
We define a Matrix Product Operator (MPO) for the 1D Transverse-field Ising Model (TFIM) and seek to optimise a Matrix Product State (MPS), such that after optimisation, 

$MPS \approx E_0$

where $E_0$ is the dominant Eigenvector of H.

The TFIM is defined as

$H = -J * \sum_{i=1}^{L-1} Sz_i * Sz_{i+1}  - g * \sum_{i=1}^{L} Sx_i$

for a `spin-1/2` system, where $S_z$ and $S_x$ are Pauli gates, $L$ is the number of spins in the chain, $J$ is the nearest neighbour in $z$ and $g$ is the transverse field along $x$.
...
begin
	# Bond dimensions [2, 4, 2] for L=4
 	maxdim = 4
	M = randomMPS(sites; linkdims=maxdim)
	M = ITensorMPS.orthogonalize(M, 1) # Right-canonical with centre at 1
	nothing
end

# ╔═╡ dcfe8017-1daf-42ab-a294-631da238020e
md"""
## Step 1a: Optimization of First Bond

We project $H$ in terms of $R_j$ tensors, ITensors handles left/right environments and canonicalization.

`eigsolve` from `KrylovKit` implements the Lanczos iterative algorithm to solve the eigenproblem.

`psi` is the bond tensor for first two sites, $B_{12}$ in the TensorNetwork tutorial
"""

# ╔═╡ 5b6f88a8-7769-4b9a-8148-d53e0e6bb231
begin
	# Using ITensor functions for effective Hamiltonian and environments
	PH = ProjMPO(H)
	position!(PH, M, 1)

	psi = M[1] * M[2] # B_12

	# KrylovKit Lanczos iterative solver
	vals, vecs = eigsolve(PH, psi, 1, :SR)

	ground_state = vecs[1] # B'_12
	energy = vals[1]
end

# ╔═╡ da53eb55-0775-4556-8a5a-30bce608893d
md"""
Alternatively, as `L` is small, we can build the left/right environments ourselves and solve the effective Hamiltonian.

$H_{eff} = L_n * H_n * H_{n+1} * R_{n+2}$

I found the diagrams [here](https://indico.icc.ub.edu/event/116/contributions/905/attachments/248/466/IntroDMRG.pdf) useful for visualising left/right environments
"""

# ╔═╡ f6d4c8db-6d99-44a6-9a64-76eaca74e5fa
# Build left env
function build_L(M, H)
	Lenv = Vector{ITensor}(undef, L+1)
	Lenv[1] = ITensor(1.0)
	for n in 1:L-1
		Lenv[n+1] = Lenv[n] * M[n] * H[n] * dag(M[n])'
	end
	return Lenv
end													  

# ╔═╡ fbbd37be-80df-494b-87d8-4e15c772a956
# Build right env
function build_R(M, H)
	Renv = Vector{ITensor}(undef, L+2)
	Renv[L+1] = ITensor(1.0)
	for n in L:-1:2
		Renv[n] = M[n]' * H[n] * dag(M[n]) * Renv[n+1]
	end
	return Renv
end		

# ╔═╡ 1c1b2b19-1556-4895-9361-fa2d8a92ac3b
begin
	Lenv = build_L(M,H)
	Renv = build_R(M,H)
	H_eff = Lenv[1] * H[1] * H[2] * Renv[3]
	new_vals, new_vecs = eigsolve(H_eff, psi, 1, :SR)
	nothing
end
...
Singular Value Decomposition (SVD) of $B'_{12}$, `ground_state` in our implementation
"""

# ╔═╡ 2126a8df-388c-4b61-9632-7e9f4f9a8336
begin
	U, S, V = svd(ground_state, siteind(M, 1); maxdim=maxdim, cutoff=1e-12)
	M[1] = U
	M[2] = S * V
	nothing
end

# ╔═╡ 204c7237-32f9-4945-8b0d-7096a00f7de7
md"""
## Step 2: Optimization and Adaptation of Second Bond

The next step is nearly identical, we just need to update the orthogonality centre and environments. 
"""

# ╔═╡ c69ceb11-83cf-4ddb-8c41-bd23a5fd14e3
begin
	# Update to site 2 using ITensor built-ins
	position!(PH, M, 2)
	# Or using our environments
	M_new = ITensorMPS.orthogonalize(M, 2)
	Lenv[2] = Lenv[1] * M_new[1] * H[1] * dag(M_new[1])'
	# psi = M[2] * M[3] etc...
	nothing
end
...
Rather than go through the whole step again, it makes sense to do a full left -> right sweep

"""

# ╔═╡ 88c4d3fa-86da-4e34-854f-c123fd9c8d6e
function lr_sweep!(M, H, maxdim; cutoff=1e-12)
    L = length(M)
    energies = Float64[]
    
    # Orthogonalize at centre = 1
    M = ITensorMPS.orthogonalize(M, 1)
    ...
    # Sweep left to right
    for i in 1:L-1
        # Move canonical center to position i
        M = ITensorMPS.orthogonalize(M, i)
        
        # Update left environment if not first step
        if i > 1
            Lenv[i] = Lenv[i-1] * M[i-1] * H[i-1] * dag(M[i-1])'
        end
        
        # Build effective Hamiltonian for bond i to i+1
        H_eff = Lenv[i] * H[i] * H[i+1] * Renv[i+2]
        
        # Form two-site wavefunction
        psi = M[i] * M[i+1]
        
        # Optimize
        vals, vecs = eigsolve(H_eff, psi, 1, :SR)
        ground_state = vecs[1]
        energy = vals[1]
        push!(energies, energy)
        
        # SVD and update tensors
        U, S, V = svd(ground_state, siteind(M, i); maxdim=maxdim, cutoff=cutoff)
        M[i] = U
        M[i+1] = S * V
    end
...
# Absolute difference between exact values and DMRG
minimum(vals_exact) - minimum(energies)
...
# ╔═╡ ba975279-f93f-41d7-af68-4c45f2278cc9
...
# ╔═╡ 3805728e-75b6-4987-b1c9-52c1c22370ca
# Run n_sweeps full sweeps
function run_dmrg(L; maxdim=4, cutoff=1e-12, n_sweeps=10)
    H, sites = get_H(L)
