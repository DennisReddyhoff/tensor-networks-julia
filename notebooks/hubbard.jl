...
begin
	...
	function HubbardSystem(L::Int, U::Float64, t::Float64)
		sites = siteinds("Electron", L)
		states = [isodd(i) ? "Up" : "Dn" for i in 1:L]
		H = OpSum()
		...
		mpo = MPO(H, sites)	
		mps = productMPS(sites, states)
	
		return HubbardSystem(L, U, t, sites, mpo, mps)
	end
end
...
function get_exact_energy(sys::HubbardSystem)
    H_dense = contract(sys.mpo)
    rows = combiner(sys.sites'; tags="row")
    cols = combiner(sys.sites;  tags="col")
